...
class CantonFairSupabaseUser extends BaseAuthUser {
  CantonFairSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> cantonFairSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CantonFairSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
